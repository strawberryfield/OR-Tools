...
*/
            ...
            XNACameraProjection = Matrix.CreateOrthographic(game.GraphicsDevice.Viewport.Width,
                game.GraphicsDevice.Viewport.Height, 1, 100);
        }
...
//                ShadowMap[shadowMapIndex] = ShadowMapRenderTarget[shadowMapIndex];
        ...
        void DrawSimple(GraphicsDevice graphicsDevice, bool logging)
        {
            ...
            //{
                if (logging) Console.WriteLine("  DrawSimple {");
                graphicsDevice.Clear(ClearOptions.Target | ClearOptions.DepthBuffer | ClearOptions.Stencil, Color.WhiteSmoke, 1, 0);
            floor.Draw(this);
                DrawSequences(graphicsDevice, logging);
                if (logging) Console.WriteLine("  }");
            //}
